...
code that is compiled in CI, so every snippet you see here is guaranteed to
type-check._
...
Here is a function that swaps the sides of a `Sum`, showing both constructors in
action:
