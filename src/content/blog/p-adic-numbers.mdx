...
is isosceles (meaning at least two of its three sides have equal length): if
$|x|_p \neq |y|_p$, then $|x + y|_p = \max(|x|_p, |y|_p)$. The inequality
becomes an equality when the two sides differ, so the third side always matches
the longer of the other two. Every open ball is also closed, because a point on
the boundary of a ball is already inside a smaller ball that fits within the
original. In fact, every point inside a $p$-adic ball is its center. There are
no boundary points in the usual sense.
