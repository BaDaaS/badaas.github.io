...
is isosceles: if $|x|_p \neq |y|_p$, then $|x + y|_p = \max(|x|_p, |y|_p)$. (The
inequality becomes an equality when the two sides differ.) Every open ball is
also closed, because a point on the boundary of a ball is already inside a
smaller ball that fits within the original. In fact, every point inside a
$p$-adic ball is its center. There are no boundary points in the usual sense.
